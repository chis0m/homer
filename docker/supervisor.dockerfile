...
COPY docker/conf/laravel-worker.conf /etc/supervisor/conf.d/laravel-worker.conf
# COPY laravel-horizon.conf.tpl /etc/supervisor/conf.d/laravel-horizon.conf.tpl
COPY docker/conf/custom-php.ini /opt/etc/custom-php.ini
COPY docker/conf/supervisord-watchdog.py /opt/supervisord-watchdog.py

# Copy scripts
COPY docker/conf/init.sh /usr/local/bin/init.sh
